...
Utils.formatAmountInSend = function(amount) {
  return Utils.formatAmount(amount, 'send', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'send';
};
...
Utils.formatUtxos = function(utxos) {
  ...
  return _.map([].concat(utxos), function(i) {
    var amount = Utils.formatAmountInSend(i.satoshis);
...
Utils.translateAddress = function(address, coin) {
  ...
  var origAddress = new Bitcore.Address(address);
  var origObj = origAddress.toObject();

  var result = Bitcore.Address.fromObject(origObj)
