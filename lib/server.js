 ...
 * @param {string} opts.coin[='send'] - The coin for this wallet (socialsend).
 ...
 * @param {string} opts.coin[='send'] - The expected coin for this wallet (socialsend).
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['send'], value.toLowerCase());
    },
  }];
 ...
 * @param {Object} opts
 * @param {String} [opts.coin='send'] (optional)
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self._getUtxosForCurrentWallet({}, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        /*utxos = utxos.filter(function(utxo){
            if(utxo.confirmations>5) return utxo;
        })
        if(utxos.length<1){
            return cb(new Error('None of your inputs have enought confirmation for an InstantSend transactions (6 required)'));
        }*/
        var validUTXO = true;
        utxos.filter(function(utxo){
            if(utxo.confirmations<6) validUTXO = false;
        })
        if(!validUTXO)
            return cb(new Error('InstantSend requires all SocialSend you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
      ...
      self._getFeePerKb(wallet, opts, function(err, feePerKb) {
        ...
        if(opts.isInstantSend===true){
             var nbInput = txp.inputs.length; 
             var feePerInput = 100000;//Fee for IS are 0.001 * INPUTS 
             txp.fee = feePerInput*nbInput; 
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'send'] - The coin to estimate fee levels from.
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  var self = this;
  ...
  function sanitizeUtxos(utxos) {
    ...
    return _.filter(utxos, function(utxo) {
      if(isInstantSend && utxo.confirmations<6) return false;     
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmountInSend(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInSend(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmountInSend(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInSend(txpAmount) + ')');
    ...
    self.logi('Big input threshold ' + Utils.formatAmountInSend(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      //log.debug('The input contributes ' + Utils.formatAmountInSend(netInputAmount));
      ...
      //log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInSend(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          //log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInSend((selected.length - 1) * feePerInput));
      ...
      //log.debug('Cumuled total so far: ' + Utils.formatAmountInSend(total) + ', Net total so far: ' + Utils.formatAmountInSend(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        //log.debug('Tx change: ', Utils.formatAmountInSend(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          //log.debug('Change below dust threshold (' + Utils.formatAmountInSend(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      //log.debug('Could not reach Txp total (' + Utils.formatAmountInSend(txpAmount) + '), still missing: ' + Utils.formatAmountInSend(txpAmount - netTotal));
  ...
  //log.debug('Selecting inputs for a ' + Utils.formatAmountInSend(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        //log.debug('Fee for this selection: ' + Utils.formatAmountInSend(fee));
      ...
      if (!err) {
        ...
        self.logi('Successfully built transaction. Total fees: ' + Utils.formatAmountInSend(txp.fee) + ', total change: ' + Utils.formatAmountInSend(change));
      } else {
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
            self._selectTxInputs(txp, opts.utxosToExclude, next, opts.isInstantSend);
          },
          function(next){
            if(opts.isInstantSend===true){
                //FIXME: May be not the best place to do this, buthas to be after selectTxInputs
                var nbInput = txp.inputs.length;
                var feePerInput = 100000;//Fee for IS are 0.001 * INPUTS
                txp.fee = feePerInput*nbInput;
                txp.isInstantSend = true;
...
WalletService.prototype._broadcastixRawTx = function(network, raw, cb) {
    var bc = this._getBlockchainExplorer(network);
    bc.broadcastix(raw, function(err, txid) {
        if (err) return cb(err);
        return cb(null, txid);
    });
};
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'send'] - The coin for this transaction.
...
/**
 * Broadcast a raw transaction for instantSend transactions.
 * @param {Object} opts
 * @param {string} [opts.network = 'livenet'] - The SocialSend network for this transaction.
 * @param {string} opts.rawTx - Raw tx data.
 */
WalletService.prototype.broadcastIXRawTx = function(opts, cb){
    var self = this;
    ...
    self._broadcastixRawTx(network, opts.rawTx, cb);
};
...
WalletService.prototype.broadcastTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      // TODO - is this broken?
      if(txp.hasOwnProperty('isInstantSend') && txp.isInstantSend===true){
        self._broadcastixRawTx(txp.getNetworkName(), raw, function(err, txid) {
          if (err) {
 ...
 * @param cb
 */
WalletService.prototype._getHeader = function(network, hash, cb) {
  var bc = this._getBlockchainExplorer(network);
 ...
 */
WalletService.prototype.syncStatus = function(network, cb) {
  var bc = this._getBlockchainExplorer(network);
  bc.syncStatus(function(err, result) {
    if (err) return cb(err);
    return cb(null, result);
  });
};
